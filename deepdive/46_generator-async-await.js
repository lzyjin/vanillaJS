...
// 이터러블이면서 동시에 이터레이터인 제너레이터 객체는 next 메세드를 갖는다.
// 제너레이터 객체의 next 메서드를 호출하면 제너레이터 함수의 코드 블록을 실행한다.

// ❗️ 단, 일반 함수처럼 한 번에 코드 블록을 일괄 실행하는 것이 아니라 yield 표현식까지만 실행한다.
// ❗️ yield 키워드는 제너레이터 함수의 실행을 일시 중지시키거나 yield 키워드 뒤에 오는 표현식의 평가 결과를 제너레이터 함수 호출자에게 반환한다.
// 다음 예제를 살펴보자.
{
    // 제너레이터 함수
    function* genFunc() {
    ...
    // next 메서드는 이터레이터 리절트 객체({vallue, done})를 반환한다.
...
// ❗️ 제너레이터 객체의 next 메서드를 호출하면 yield 표현식까지 실행되고 일시 중지(suspend)된다.
// 이때 함수의 제어권이 호출자로 양도(yield)된다.
// 이후 필요한 시점에 호출자가 또다시 next 메서드를 호출하면 일시 중지된 코드부터 실행을 재개(resume)하기 시작하여 다음 yield 표현식까지 실행되고 또
// 다시 일시 중지된다.

// 이때 제너레이터 객체의 next 메서드는 value, done 프로퍼티를 갖는 이터레이터 리절트 객체를 반환한다.
// next 메서드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에는 yield 표현식에서 yield된 값(yield 키워드 뒤의 값)이 할당되고
// done 프로퍼티에는 제너레이터 함수가 끝까지 실행되었는지를 나타내는 불리언 값이 할당된다.

// 이처럼 next 메서드를 반복 호출하여 yield 표현식까지 실행과 일시 중지를 반복하다가 제너레이터 함수가 끝까지 실행되면 next 메서드가 반환하는
// 이터레이터 리절트 객체의 value 프로퍼티에는 제너레이터의 반환값이 할당되고 done 프로퍼티에는 제너레이터 함수가 끝까지 실행되었음을 나타내는 true가 할당된다.
...
// 다음 예제를 살펴보자.
{
    function* genFunc() {
        ...
        // ❗️ 두 번째 next 메서드를 호출할 때 전달한 인수 10은 첫 번째 yield 표현식을 할당받는 x 변수에 할당된다.
        // 즉, const x = yield 1;은 두 번째 next 메서드를 호출했을 때 완료된다.
        // 두 번째 next 메서드를 호출하면 두 번째 yield 표현식까지 실행되고 일시 중지된다.
        // 이때 yield된 값 x + 10은 next 메서드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에 할당된다.
        const y = yield (x + 10);

        // ❗️ 세 번째 next 메서드를 호출할 때 전달한 인수 20은 두 번째 yield 표현식을 할당받는 y 변수에 할당된다.
        // 즉, const y = yield (x + 10);는 세 번째 next 메서드를 호출했을 때 완료된다.
        // 세 번째 next 메서드를 호출하면 함수 끝까지 실행된다.
        // 이때 제너레이터 함수의 반환값 x + y는 next 메서드가 반환한 이터레이터 리절트 객체의 value 프로퍼티에 할당된다.
        // ❗️ 일반적으로 제너레이터의 반환값은 의미가 없다.
        // ❗️ 따라서 제너레이터에서는 값을 반환할 필요가 없고 return은 종료의 의미로만 사용해야 한다.
        return x + y;
    }
    ...
    // ❗️ 처음 호출하는 next 메서드에는 인수를 전달하지 않는다.
    // ❗️ 만약 처음 호출하는 next 메서드에 인수를 전달하면 무시된다.
...
// next 메서드에 인수를 전달해서 제너레이터 객체에 상태(yield 표현식을 할당하는 변수)를 밀어넣을 수 있다.
// 이러한 제너레이터의 특성을 활용하면 비동기 처리를 동기 처리처럼 구현할 수 있다.
// 이에 대해서는 46.5.2절 "비동기 처리"에서 자세히 살펴보자.
...
// 먼저 이터레이션 프로토콜을 준수하여 무한 피보나치 수열을 생성하는 함수를 구현해 보자.
{
    // 무한 이터러블을 생성하는 함수
    ...
    for (const num of infiniteFibonacci) {
        ...
        console.log(num); // 1 2 3 5 8 13 ... 2584 4181 6765
    }
}
...
// 다시 말해, 프로미스의 후속 처리 메서드 then/catch/finally 없이 비동기 처리를 반환하도록 구현할 수 있다.
// 다음 예제를 살펴보자.
{
    ...
    (async(function* fetchTodo() { // (1)
        ...
        // ❗️fetch()가 반환한 Response 객체를 래핑한 Promise 객체는 HTTP 응답 전체를 나타내는 객체로,
        //   JSON 본문 콘텐츠를 추출하기 위해서는 json() 메서드를 호출해야 한다.
        // ❗️json()은 응답 본문 텍스트를 JSON으로 파싱한 결과로 이행하는 또 다른 프로미스를 반환한다.
...
//    onResolved 함수는 상위 스코프의 generator를 기억하는 클로저다.
//    async 함수가 반환한 onResolved 함수를 즉시 호출(4)하여 (2)에서 생성한 제너레이터 객체의 next 메서드를 처음 호출(5)한다.

// 2. next 메서드가 처음 호출(5)되면 제너레이터 함수 fetchTodo의 첫 번째 yield문(6)까지 실행된다.
//    이때 next 메서드가 반환한 이터레이터 리절트 객체의 done 프로퍼티 값이 false, 즉 아직 제너레이터 함수가 끝까지 실행되지 않았다면 이터레이터 리절트
//    객체의 value 프로퍼티 값, 즉 첫 번째 yield된 fetch 함수가 반환한 프로미스가 resolve한 Response 객체를 onResolved 함수에 인수로
//    전달하면서 재귀 호출(7) 한다.

// 3. onResolved 함수에 인수로 전달된 Response 객체를 next 메서드에 인수로 전달하면서 next 메서드를 두 번째로 호출(5)한다.
//    이때 next 메서드에 인수로 전달한 Response 객체는 제너레이터 함수 fetchTodo의 response 변수(6)에 할당되고 제너레이터 함수 fetchTodo의
//    두 번째 yield문(8)까지 실행된다.

// 4. next 메서드가 반환한 이터레이터 리절트 객체의 done 프로퍼티 값이 false, 즉 아직 제너레이터 함수 fetchTodo가 끝까지 실행되지 않았다면
//    이터레이터 리절트 객체의 value 프로퍼티 값, 즉 두 번째 yield된 response.json 메서드가 반환한 프로미스가 resolve한 todo 객체를
...
// 위 예제의 제너레이터 함수를 실행하는 제너레이터 실행기인 aysnc 함수는 이해를 돕기 위한 간략화된 예제이므로 완전하지 않다.
// 다음에 설명할 async/await를 사용하면 async 함수와 같은 제너레이터 실행기를 사용할 필요가 없지만 혹시 제너레이터 실행기가 필요하다면 직접 구현하는
// 것보다 co 라이브러리를 사용하기 바란다.
{
...
// ❗️ async/await를 사용하면 프로미스의 then/catch/finally 후속 처리 메서드에 콜백 함수를 전달해서 비동기 처리 결과를 후속 처리 할 필요 없이 마치
// 동기 처리처럼 프로미스를 사용할 수 있다.
// ❗️ 다시 말해, 프로미스의 후속 처리 메서드 없이 마치 동기 처리처럼 프로미스가 처리 결과를 반환하도록 구현할 수 있다.
...
// ❗️ async 함수는 async 키워드를 사용해 정의하며 언제나 프로미스를 반환한다.
...
// ❗️클래스의 constructor 메서드는 async 메서드가 될 수 없다.
...
// 📌 await 키워드
// ❗️await 키워드는 프로미스가 settled 상태(비동기 처리가 수행된 상태)가 될 때까지 대기하다가
...
// await 키워드는 반드시 프로미스 앞에서 사용해야 한다.
{
    ...
    getGithubUserName('lzyjin'); // YEJIN
}

// await 키워드는 프로미스가 settled 상태가 될 때까지 대기한다고 했다.
// 따라서 (1)의 fetch 함수가 수행한 HTTP 요청에 대한 서버의 응답이 도착해서 fetch 함수가 반환한 프로미스가 settled 상태가 될 때까지
// (1)은 대기하게 된다.
...
// 첫 번째 프로미스는 settled 상태가 될 때까지 3초, 두 번째 프로미스는 settled 상태가 될 때까지 2초, 세 번째 프로미스는 settled 상태가 될 때까지
// 1초가 소요되기 때문이다.

// 그런데 foo 함수가 수행하는 3개의 비동기 처리는 서로 연관이 없이 개별적으로 수행되는 비동기 처리이므로 앞선 비동기 처리가 완료될 때까지 대기해서
// 순차적으로 처리할 필요가 없다.
...
// 따라서 비동기 처리의 처리 순서가 보장되어야 하므로 모든 프로미스에 await 키워드를 써서 순차적으로 처리할 수 밖에 없다.
{
    async function bar(n) {
        ...
        const b = await new Promise(resolve => setTimeout(() => resolve(a + 10), 2000));

        // 세 번째 비동기 처리를 수행하려면 첫 번째 비동기 처리 결과가 필요하다.
        const c = await new Promise(resolve => setTimeout(() => resolve(b + 10), 1000));

        console.log([a, b, c]); // [1, 11, 21]
    }
...
// 45.1.2절 "에러 처리의 한계"에서 살펴본 바와 같이 에러는 호출자(caller) 방향으로 전파된다.
// 즉, 콜 스택의 아래 방향(실행 중인 실행 컨텍스트가 푸쉬되기 직전에 푸쉬된 실행 컨텍스트 방향)으로 전파된다.
...
// ❗️async/await에서 에러 처리는 try...catch 문을 사용할 수 있다.
// ❗️콜백 함수를 인수로 전달받는 비동기 함수와는 달리 프로미스를 반환하는 비동기 함수는 명시적으로 호출할 수 있기 때문에 호출자가 명확하다.
{
    const foo = async () => {
        try {
...
// ❗️async 함수 내에서 catch 문을 사용해서 에러 처리를 하지 않으면 async 함수는 발생한 에러를 reject하는 프로미스를 반환한다.
// ❗️따라서 async 함수를 호출하고 Promise.prototype.catch 후속 처리 메서드를 사용해 에러를 캐치할 수도 있다.
// 46장 제너레이터와 async/await
...
// ❗️따라서 async 함수를 호출하고 Promise.prototype.catch 후속 처리 메서드를 사용해 에러를 캐치할 수도 있다.
{
    ...
    foo()
        .then(console.log)
        .catch(console.error); // TypeError: Failed to execute 'fetch' on 'Window': Failed to parse URL
