...
// 13.5절 "렉시컬 스코프"에서 살펴보았던 예제를 다시 살펴보자.
{
  const x = 1;
  function foo() {
    const x = 10;
...
// 함수가 호출되면 함수 내부로 코드의 제어권이 이동한다. 그리고 코드를 평가하기 시작한다.
...
// 즉 outer 함수의 실행이 종료되면 outer 함수의 실행 컨텍스트는 실행 컨텍스트 스택에서 제거pop된다.
...
// ❕ 그러나 위 코드의 실행 결과(4번)는 outer 함수의 지역 변수 x 값인 10이다.
...
// 24.1절 "렉시컬 스코프"에서 살펴보았듯이 위 정의에서 "그 함수가 선언된 렉시컬 환경"이란 함수가 정의된 위치의 스코프, 즉 상위 스코프를 의미하는 실행 컨텍스트의 렉시컬 환경을 말한다.
...
// 위 예제로 돌아가보자.
// 위 예제에서 outer 함수가 평가되어 함수 객체를 생성할 때(1번) 현재 실행 중인 실행 컨텍스트의 렉시컬 환경, 즉 전역 렉시컬 환경을 outer 함수 객체의 [[Encironment]] 내부 슬롯에 상위 스코프로서 저장한다.
...
// 2번 inner 함수는 함수 표현식으로 정의했기 때문에 런타임에 평가된다.
// 이때 중첩 함수 inner는 자신의 [[Environment]] 내부 슬롯에 현재 실행중인 실행 컨텍스트의 렉시컬 환경,
...
// outer 함수의 렉시컬 환경은 inner 함수의 [[Envifonment]] 내부 슬롯에 의해 참조되고 있고, inner 함수는 전역 변수 innerFunc에 의해 참조되고 있으므로 가비지 컬렉션의 대상이 되지 않기 때문이다.
// 가비지 컬렉터는 누군가가 참조하고 있는 메모리 공간을 함부로 해제하지 않는다.

// outer 함수가 반환한 inner 함수를 호출(4번)하면 inner 함수의 실행 컨텍스트가 생성되고, 실행 컨텍스트 스택에 푸쉬된다.
// 그리고 렉시컬 환경의 외부 렉시컬 환경에 대한 참조에는 inner 함수 객체의 [[Environment]] 내부 슬롯에 저장되어 있는 참조값이 할당된다.

// 중첩함수 inner는 외부 함수 outer보다 더 오래 생존했다.
...

{
  ...
  // 중첩 함수 bar는 클로저였지만 외부 함수보다 일찍 소멸되기 때문에 클로저의 본질에 부합하지 않는다.

  // 위 예제의 중첩 함수 bar는 상위 스코프의 식별자를 참조하고 있으므로 클로저다.
  // 하지만 외부 함수 foo의 외부로 중첩함수 bar가 반환되지 않는다.
  // 즉, 외부 함수 foo보다 중첩 함수 bar의 생명 주가기 짧다.
  // 이런 경우 중첩 함수 bar는 클로저였지만 외부 함수보다 일찍 소멸되기 때문에 생명 주기가 종료된 외부 함수의 식별자를 참조할 수 있다는 클로저의 본질에 부합하지 않는다. 따라서 중첩 함수 bar는 일반적으로 클로저라고 하지 않는다.
}
...
// 이런 경우 대부분의 모던 브라우저는 최적화를 통해 그림 24-8과 같이 상위 스코프의 식별자 중에서 클로저가 참조하고 있는 식별자만을 기억한다.

// 클로저에 의해 참조되는 상위 스코프의 변수(위 예제의 경우 foo 함수의 x변수)를 자유변수(free variable)라고 부른다.
// 클로저란 "함수가 자유 변수에 대해 닫혀있다(closed)"라는 의미다.
// 이를 좀 더 알기 쉽게 의역하자면 "자유 변수에 묶여있는 함수"라고 할 수 있다.

// 이론적으로 클로저는 상위 스코프를 기억해야 하므로 불필요한 메모리의 점유를 걱정할 수도 있겠다.
...
// 클로저가 유용하게 사용되는 상황을 살펴보자.

// 👉 24.4 클로저의 활용
// 클로저는 상태state를 안전하게 변경하고 유지하기 위해 사용한다.
// 다시 말해, 상태가 의도치 않게 변경되지 않도록 상태를 안전하게 은닉(information hiding)하고 특정 함수에서만 상태 변경을 허용하기 위해 사용한다.

// 함수가 호출될 때마다 호출된 횟수를 누적하여 출력하는 카운터를 만들어보자.
// 이 예제의 호출된 횟수(num 변수)가 바로 안전하게 변경되고 유지해야 할 상태다.
{
  // 카운트 상태 변수
  let num = 0;
...
// 1)번 num은 생성자 함수 Counter가 생성할 인스턴스의 프로퍼티가 아니라 즉시 실행 함수 내에서 선언된 변수다.
// 만약 num이 생성자 함수 Counter가 생성할 인스턴스의 프로퍼티라면(2)번) 인스턴스를 통해 외부에서 접근이 자유로운 public 프로퍼티가 된다.
...
// 그리고 makeCounter 함수는 함수 객체를 생성하여 반환 후 소멸된다.
...
// 👉 24.5 캡슐화와 정보 은닉
// 캡슐화(encapsulation)는 객체의 상태를 표현하는 프로퍼티와 프로퍼티를 참조하고 조작할 수 있는 동작인 메서드를 하나로 묶은 것을 말한다.
...
// ❗️ 하지만 위 예제의 sayHi 메서드는 인스턴스 메서드이므로 Person 객체가 생성될 때마다 중복 생성된다.
...

{
  ...
  console.log( me.sayHi() ); // 1) Hi! My name is Lee. I am 30
}
...
// 하지만 위 코드도 문제가 있다.
// Person 생성자 함수가 여러 개의 인스턴스를 생성할 경우 다음과 같이 _age 변수의 상태가 유지되지 않는다는 것이다.:  1)번
...
// 인스턴스 메서드를 사용한다면 자유 변수를 통해 private를 흉내낼 수는 있지만 프로토타입 메서드를 사용하면 이마저도 불가능해진다. ..
...
// 아래는 클로저를 사용할 때 자주 발생할 수 있는 실수를 보여주는 예제다
{
  ...
  for(var j = 0; j < funcs.length; j++) {
    console.log( funcs[j]() ); // 3 3 3 // 2)
  }
}
// 첫 번째 for문의 코드 블록 내(1번)에서 함수가 funcs 배열의 요소로 추가된다.
// 그리고 두 번째 for문의 코드 블록 내(2번)에서 funcs 배열의 요소로 추가된 함수를 순차적으로 호출한다.
// 이때 funcs 배열의 요소로 추가된 3개의 함수가 0, 1, 2를 반환할 것으로 기대했다면 아쉽지만 결과는 그렇지 않다.

// for 문의 변수 선언문에서 var 키워드로 선언한 i 변수는 블록레벨 스코프가 아닌 함수 레벨 스코프를 갖기 때문에 전역 변수다.
// 전역 변수 i에는 0, 1, 2가 순차적으로 할당된다..
// 따라서 funcs 배열의 요소로 추가한 함수를 호출하면 전역 변수 i를 참조하여 i의 값 3가 출력된다.
...
// 1)에서 즉시 실행 함수는 전역 변수 i에 현재 할당되어 있는 값을 인수로 전달받아 매개변수 id에 할당한 후 중첩 함수를 반환하고 종료한다.
...
// 만약 for 문의 코드 블록 내에서 정의한 함수가 있다면 이 함수의 상위 스코프는 for문의 코드 블록이 실행될 때마다 생성된 for문 코드 블록의 새로운 렉시컬 환경이다.
// 이때 함수의 상위 스코프는 for문의 코드 블록이 반복 실행될 때마다 식별자(for 문의 변수 선언문에서 선언한 초기화 변수 및 for 문의 코드 블록 내에서 선언한 지역 변수)의 값을 유지해야 한다.
// 이를 위해 for문이 반복될 대마다 독립적인 렉시컬 환경을 생성하여 식별자의 값을 유지한다.

// 이처럼 let이나 const 키워드를 사용하는 반복문(for, for...in, for...of, whie)은 코드 블록을 반복 실행할 때마다 새로운 렉시컬 환경을 생성하여 반복할 당시의 상태를 마치 스냅숏을 찍는 것처럼 저장한다.
