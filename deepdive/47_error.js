...
// querySelector나 Array#find 메서드처럼 예외적인 상황이 발생하면 반환하는 값(null 또는 -1)을 if문이나 단축 평가 또는 옵셔널 체이닝 연산자를 통해
// 확인해서 처리하는 방법과
...
// 이때 try 코드 블록에 포함된 문 중에서 에러가 발생하면 발생한 에러는 catch 문의 err 변수에 전달되고 catch 블록이 실행된다.
...
// try...catch...finally 문으로 에러를 처리하면 프로그램이 강제 종료되지 않는다.
{
    ...
    // 출력:
    // [시작]
    // 47_error.js:99 ReferenceError: foo is not defined
    // [마침내]
    // [끝]
}
...
// SyntaxError, ReferenceError, TypeError, RangeError, URIError, EvalError 생성자 함수가 생성한 에러 객체의 프로토타입은 모두
// Error.prototype을 상속받는다.
...
// 👉 throw 문
// ❗️ Error 생성자 함수로 에러 객체를 생성한다고 해서 에러가 발생하는 것은 아니다.
// ❗️ 즉, 에러 객체 생성과 에러 발생은 의미가 다르다.
{
    try {
...
// ❗️ 에러를 발생시키려면 try 코드 블록에서 throw 문으로 에러 객체를 던져야 한다.
{
    // throw 표현식;
}
...
// 👉 에러의 전파
// 45장 "프로미스"의 45.12절 "에러 처리의 한계"에서 살펴본 바와 같이 에러는 호출자(caller) 방향으로 전파된다.
...
// 다음 예제를 살펴보자.
{
    ...
    try {
        ...
        console.error(e); // Error: foo에서 발생한 에러
    }
}
// ❗️ (1)에서 baz 함수를 호출하면 (2)에서 bar 함수가 호출되고 (3)에서 foo 함수가 호출되고 foo 함수는 (4)에서 에러를 throw한다.
// ❗️ 이때 foo 함수가 throw한 에러는 다음과 같이 호출자에게 전파되어 전역에서 캐치된다.

// 이처럼 throw된 에러를 캐치하지 앟으면 호출자 방향으로 전파된다.
// 이때 throw된 에러를 캐치하여 적절히 대응하면 프로그램을 강제로 종료시키지 않고 코드의 실행 흐름을 복구할 수 있다.
...
// 콜 스택으로 푸쉬되어 실행된다.
// 이때 콜 스택에 푸쉬된 콜백 함수의 실행 컨텍스트는 콜 스택의 가장 하부에 존재하게 된다.
