...
// 즉 변수는 생성되고 소멸되는 생명 주기life cycle가 있다.
...
// foo 함수를 호출하지 않으면 함수 내부에 선언문이 실행되지 않기 때문이다.

// "변수 선언의 실행 시점과 변수 호이스팅"에서 말한 "변수 선언은 선언문이 어디에 있든 상관없이 가장 먼저 실행된다." 이말은 전역 변수에 한정된 것이다.
// 함수 내부에 선언한 변수는 함수가 호출된 직후에 함수 몸체의 코드가 한 줄씩 순차적으로 실행되기 이전에 자바스크립트 엔진에 의해 먼저 실행된다.

// 위 예제의 foo 함수를 호출하면 함수 몸체의 다른 문들이 순차적으로 실행되기 이전에 x 변수의 선언문이 자바스크립트 엔진에 의해 가장 먼저 실행되어 x변수가 선언되고 undefined로 초기화된다.
// 그 후 함수 몸체를 구성하는 문들이 순차적으로 실행되기 시작하고, 변수 할당문이 실행되면 x 변수에 값이 할당된다.
// 함수가 종료하면 x 변수도 소멸되어 생명 주기가 종료된다.
// = 함수 내부에 선언된 지역 변수 x는 foo 함수가 호출되어 실행되는 도안에만 유효하다.
// = 지역 변수의 생명 주기는 함수의 생명 주기와 일치한다.

// 함수 몸체 내부에서 선언된 지역 변수의 생명 주기는 함수의 생명 주기와 대부분 일치하지만, 지역 변수가 함수보다 오래 생존하는 경우도 있다.
// 함수 내부에서 선언된 지역 변수는 함수가 생성된 스코프에 등록된다.
...
var x = 'global';
function foo() {
  console.log(x); // 1) 여기는 무슨값일까? 내답변: undefined 딩동댕~
  var x = 'local';
}
foo();
console.log(x); // global

// foo 함수 내부에서 선언된 지역 변수 x는 1)의 시점에 이미 선언되었고, undefined로 초기화되어 있다.
...
// 다시 말해, 전역 코드는 함수 호추과 같이 전역 코드를 실행하는 특별한 진입점이 없고 코드가 로드되자마자 곧바로 해석되고 실행된다.
...
// 이는 모든 코드가 전역 변수를 참조하고 변경할 수 있는 암묵적 결합implict coupling을 허용하는 것이다.
...
// 검색 속도 차이는 그다지 크지 않지만 속도의 차이는 분명히 있다.
...
// 네임스페이스 객체에 또다른 네임스페이스 객체를 프로퍼티로 추가해서 네임스페이스를 계층적으로 구성할 수도 있다.
var MYAPP = {}; // 전역 네임스페이스 객체
MYAPP.person = {
  name: 'Lee',
  address: 'Seoul'
...
// 캡슐화encapsultion는 객체의 상태를 나타내는 프로퍼티와, 프로퍼티를 참조하고 조작할 수 있는 동작인 메서드를 하나로 묶는 것을 말한다.
// 캡슐화는 객체의 특정 프로퍼티나 메서드는 감출 목적으로 사용하기도 하는데 이를 정보 은닉information hiding이라 한다.
