...
// 웹 브라우저를 벗어나 서버 사이드 애플리케이션 개발에서도 사용할 수 잇는 범용 개발 언어가 되었다.
...
// 이를 위해 브라우저가 HTML, CSS, 자바스크립트로 작서오딘 텍스트 문서를 어떻게 파싱(헤석)하여 브라우저에 렌더링하는지 살펴보자.
...
// 토큰에 문법적 의미와 구조를 반영하여 트리 구조의 자료구조인 파스 트리(parse tree/syntax tree)를 생성하는 일련의 과정
...
// 서버에 요청(request)하고 서버로부터 응답(response)받아 브라우저에 시각적으로 렌더링하는 것이다.
...
// 예를 들어 브라우저의 주소창에 https://www.poiemaweb.com 을 입력하고 엔터 키를 누르면
...
// 즉, https://www.poiemaweb.com 은 https://www.poiemaweb.com/index.html과 같은 요청이다.

// 따라서 서버는 루트 요청에 대해 서버의 루트 폴더에 존재하는 정적 파일 index.html을 클라이언트(브라우저)로 응답한다.
// 만약 index.htmldㅣ 아닌 다른 정적 파일을 서버에 요청하려면
// 브라우저의 주소창에 https://www.poiemaweb.com/assets/data/data.json 과 같이 요청할 정적 파일의 경로(서버의 루트 폴더 기준)와 파일 이름을
// URI의 호스트 뒤의 패스(path)에 기술하여 서버에 요청한다.
...
// - Ajax, REST API 장에서 자세히 보자.

// ❕ 요청과 응답은 개발자 도구의 Network 패널에서 확인할 수 있다.
// 브라우저의 주소창에 https://www.poiemaweb.com 을 입력하고 엔터 키를 눌러 서버에 루트 요청을 전송해 보자.
// 다음 그림처럼 브라우저가 www.poiemaweb.com 서버에 요청한 내용과, 서버가 응답한 내용을 개발자 도구의 Network 패널에서 확인할 수 있다.
// 개발자 도구의 Network 패널을 활성화하기 이전에 브라우저가 이미 응답을 받은 경우 응답된 리소스가 표시되지 않는다.
// 따라서 Network 패널에 아무런 리소스가 표시되지 않았다면 페이지를 새로고침해야 한다.

// Network 패널을 살펴보면 index.html(poiemaweb.com)뿐만 아니라 CSS, 자바스크립트, 이미지, 폰트 파일들도 응답된 것을 확인할 수 있다.
// 요청도 하지 않은 이 리소스들은 왜 응답되었을까?

// ❕ 이는 브라우저의 렌더링 엔진이 HTML(index.html)을 파싱하는 도중에 외부 리소스를 로드하는 태그, 즉 CSS 파일을 로드하는 link 태그,
// 이미지 파일을 로드하는 img 태그, 자바스크립트를 로드하는 script 태그 등을 만나면 HTML 파싱을 일시 중단하고 해당 리소스 파일을 서버로 요청하기 때문이다.



// 👉 HTTP 1.1과 HTTP 2.0
// HTTP(HyperText Transfer Protocol)는 웹에서 브라우저와 서버가 통신하기 위한 프로토콜(규약)이다.
// 1989년, HTML, URL과 함께 팀 버너스 리 경이 고안한 HTTP는
// 1991년 최초로 문서화, 1996년 HTTP/1.0, 1999년 HTTP/1.1, 2015년 HTTP/2 가 발표되었다.

// HTTP/1.1과 HTTP/2의 차이
// HTTP/1.1은 기본적으로 커넥션(connection)당 하나의 요청과 응답만 처리한다.
// 즉, 여러 개의 요청을 한 번에 전송할 수 없고 응답 또한 마찬가지다.
// 따라서 HTML 문서 내에 포함된 여러 개의 리소스 요청, 즉 CSS 파일을 로드하는 link 태그, 이미지 파일을 로드하는 img 태그,
// 자바스크립트를 로드하는 script 태그 등에 의한 리소스 요청이 개별적으로 전송되고 응답 또한 개별적으로 전송된다.
// 이처럼 HTTP/1.1은 리소스의 동시 전송이 불가능한 구조이므로 요청할 리소스의 개수의 비례하여 응답 시간도 증가하는 단점이 있다.
...
//    참고로 meta 태그의 charset 어트리뷰트에 선언된 인코딩 방식(예: UTF-8)은
//    응답 헤더(response header)에 content-type: text/html; charset=utf-8과 같이 담겨 응답된다.
//    브라우저는 이를 확인하고 문자열로 변환한다.
// 3. 문자열로 변환된 HTML 문서를 읽어 들여 문법적 의미를 갖는 코드의 최소 단위인 토큰(token)들로 분해한다.
// 4. 각 토큰들을 객체로 변환하여 노드(node)들을 생성한다.
//    토큰의 내용에 따라 문서 노드, 어트리뷰트 노드, 텍스트 노드가 생성된다.
...
// 그리고 link 태그의 href 어트리뷰트에 지정된 CSS 파일을 서버에 요청하여 로드한 CSS 파일이나 style 태그 내의 CSS를
// HTML과 동일한 파싱 과정(바이트->문자->토큰->노드->DOM)을 거치며 해석하여 CSSOM(CSS Object Model)을 생성한다.
// 이후 CSS 파싱을 완료하면 HTML 파싱이 중단된 시점부터 다시 HTML을 파싱하기 시작하여 DOM 생성을 재개한다.

// 앞에서 살펴본 index.html을 다시 살펴보자.
// index.html에는 CSS 파일을 로드하는 link 태그가 있다.
...
// ❕ 따라서 브라우저 화면에 렌더링되지 않는 노드(예: meta 태그, script 태그)와 CSS에 의해 비표시(예: display: none)되는 노드들은 포함하지 않는다.
// 다시 말해, 렌더 트리는 브라우저 화면에 렌더링되는 노드만으로 구성된다.

// 이후 완성된 렌더 트리는 각 HTML 요소의 레이아웃(위치와 크기)을 계산하는데 사용되며,
...
// - HTML 요소의 레이아웃(위치, 크기)에 변경을 발생시키는 width/height, margin, padding, border,
// display, position, top/left/right/bottom 등의 스타일 변경
...
// 👉 자바스크립트 파싱과 실행
// HTML 문서를 파싱한 결과물로서 DOM은 HTML 문서의 구조와 정보뿐만 아니라
// HTML 요소와 스타일 등을 변경할 수 있는 프로그래밍 인터페이스로서 DOM API를 제공한다.
// 즉, 자바스크립트 코드에서 DOM API를 사용하면 이미 생성된 DOM을 동적으로 조작할 수 있다.
// DOM API를 사용하여 DOM을 조작하는 방법은 39장 "DOM"에서 살펴보자.

// CSS 파싱 과정과 마찬가지로 렌더링 엔진은 HTML을 한 줄씩 순차적으로 파싱하여 DOM을 생성해 나가다가
// 자바스크립트 파일을 로드하는 script 태그나 자바스크립트 코드를 콘텐츠로 담은 script 태그를 만나면 DOM 생성을 일시 중단한다.

// 그리고 script 태그의 src 어트리뷰트에 정의된 자바스크립트 파일을 서버에 요청하여 로드한 자바스크립트 파일이나
// script 태그 내에 자바스크립트 코드를 파싱하기 위해 자바스크립트 엔진에 제어권을 넘긴다.
...
// 렌더링 엔진인 HTML과 CSS를 파싱하여 DOM과 CSSOM을 생성하듯이,
...
//                토크나이저(Tokenizer)        파서(Parser)               바이트코드 생성기(Bytecodegenerator)             인터프리터(Interpreter)
//                토크나이징(Tokenizing)       파싱(Parsing)                        바이트코드 생성                          실행(Execution)
//                    ->                       ->                                    ->                                   ->
// 자바스크립트 소스코드             토큰(Token)               AST(Abstract Syntax Tree)            바이트코드(Bytecode)
// ------------ 어휘 분석 ----------- | --------------------------- 구문 분석 --------------------------------------
...
// 참고로 V8 엔진의 경우 자주 사용되는 코드는 터보팬(TurborFan)이라 불리는 컴파일러에 의해 최적화된 머신 코드로 컴파일되어 성능을 최적화한다
...
// 리플로우: 레이아웃 계산을 다시 하는 것. 노드 추가/삭제, 요소의 크기/위치 변경, 윈도우 리사이징 등 레이아웃에 영향을 주는 변경이 발생한 경우에 한해 실행된다.
...
//  <head>                                              |
//   <meta charset="UTF-8">                             ↓
//   <link rel="stylesheet" href="style.css">          HTML      ↓ CSS 파싱
//   <script src="app.js"></script>                   파싱 중단    ↓ JavaScript 파싱/실행
//  </head>                                             |
//  <body>                                              |
//   <ul>                                               |
//    <li id="apple">Apple</li>                         |
//    <li id="banana">Banana</li>                       | HTML 파싱
//    <li id="orange">Orange</li>                       |
//   </ul>                                              |
//   <script src="app.js"></script>                     |
//  </body>                                             |
...
// 만약 DOM을 변경하는 DOM API를 사용할 때 DOM 생성이 완료되지 않은 상태라면 문제가 발생할 수 있다.

// 다음 예제를 살펴보자.
// 다음 자바스크립트 코드의 동작 방식을 지금 당장 이해할 필요는 없다.
// 지금 주목할 것은 script 태그의 위치에 의해 블로킹이 발생한다는 것이다.
// 38_example-bad.html 참고

// DOM API인 document.getElementById('apple')은 DOM에서 id가 'apple'인 HTML 요소를 취득한다.
// document.getElementById('apple')을 실행하는 시점에는 아직 id가 'apple'인 HTML 요소를 파싱하지 않았기 때문에,
// DOM에는 id가 'apple'인 HTML 요소가 포함되어 있지 않은 상태다.
// 따라서 위 예제는 정상적으로 동작하지 않는다.

// 이러한 문제를 회피하기 위해 body 요소의 가장 아래에 자바스크립트를 위치하는 것은 좋은 아이디어다.
// 이유:
// - DOM이 완성되지 않은 상태에서 자바스크립트가 DOM을 조작하면 에러가 발생할 수 있다.
// - 자바스크립트 로딩/파싱/실행으로 인해 HTML 요소들의 렌더링에 지장받는 일이 발생하지 않아 페이지 로딩 시간이 단축된다.

// 위 예제의 자바스크립트를 body 요소의 가장 아래, 즉 닫는 body 태그 바로 위에 위치시켜 보자.
// 38_example-good.html 참고
...
// 앞에서 살펴본 자바스크립트 파싱에 의한 DOM 생성이 중단(blocking)되는 문제를 근본적으로 해결하기 위해
...
// async와 defer 어트리뷰트를 사용하면 HTML 파싱과 외부 자바스크립트 파일의 로드가 비동기적(asynchronous)으로 동시에 진행된다.
// 하지만 자바스크립트의 실행 시점에 차이가 있다.

// 🌷 async 어트리뷰트
// HTML 파싱과 외부 자바스크립트 파일의 로드가 비동기적으로 동시에 진행된다.
// 단, 자바스크립트의 파싱과 실행은 자바스크립트의 파일이 로드된 직후 진행되며, 이때 HTML 파싱이 중단된다.

//                   JavaScript 로드    JavaScript 실행
//                 ● --------------> ● -------------->
// ● --------------------------------                 --------------------→
...
// 따라서 DOM 생성이 완료된 직후 실행되어야 할 자바스크립트에 유용하다.
// defer 어트리뷰트는 IE10 이상에서 지원된다.
// IE6 ~ 9 에서도 지원되기는 하지만 정상적으로 동작하지 않을 수 있다.
